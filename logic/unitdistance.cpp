...
UnitDistance::UnitDistance(int _ID): Unit(_ID) {
	m_state = STATE_WALK;
	SelectAnim("Walk");
	
	// DEFENSE PARAMETERS
	m_attack = 5; //int
	m_attackRate = 200; //int
	m_defense = 0; //int
	m_health = 10; //int
	m_visionRange = 25.0f; //float
	m_attackRange = 18.0f; //float
	m_speed  = 0.12f; //float
...
SpriteAnimData *UnitDistance::unitData = NULL;
void UnitDistance::loadContent()
{
	Unit::loadContent(unitData, "data/scripts/unit_distance.anim");
}

void UnitDistance::unloadContent()
{
		...
		std::queue<Event*>& _eventQueue)
{
	...
	//Distance carrega el atac sempre
	if(m_attackRateCount < m_attackRate) m_attackRateCount++;
	...
	switch(m_state){
			...
			for(unsigned int i = 0; i < _units.size(); ++i){
				...
				if(_units[i]->getTeam() != m_team &&
				   _units[i]->getHealth() > 0) {
					...
					//Si no esta dins del camp de atac (distance), ignorem
		...
		case STATE_ATTACK: {
			...
			if(m_enemyUnit == NULL || m_enemyUnit->getHealth() <= 0 || !inAttack(m_enemyUnit->position())) {
				...
				//recordem que Distance carrega el atac sempre
...
int UnitDistance::hurt (int _attack, int _origin) {
...
#ifdef _DEBUG
void UnitDistance::assertStateChanges(int _numStateChanges, int* _statesStack)
{
	fprintf(stderr, "UnitDistance assert: too many state changes in one update\n\n");
