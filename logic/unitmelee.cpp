...
UnitMelee::UnitMelee(int _ID): Unit(_ID) {
	m_state = STATE_WALK;
	SelectAnim("Walk");
...
SpriteAnimData *UnitMelee::unitData = NULL;
void UnitMelee::loadContent()
{
	Unit::loadContent(unitData, "data/scripts/unit_melee.anim");
}

void UnitMelee::unloadContent()
{
	Unit::unloadContent(unitData);
}

void UnitMelee::update (
...
#define MAX_STATE_CHANGES 32
	...
	if (StateChanges >= MAX_STATE_CHANGES)
		...
		break;
		case STATE_CHASE: m_attackRateCount++; //Melee va carregant el atac en chase.
...
int UnitMelee::hurt (int _attack, int _origin) {
...
#ifdef _DEBUG
void UnitMelee::assertStateChanges(int _numStateChanges, int* _statesStack)
{
	fprintf(stderr, "UnitMelee assert: too many state changes in one update\n\n");
